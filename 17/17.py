...
# Start direction could be either right [0, 1] or down [1, 0], arbitrary choice.
start = toVisit(0, np.array([0, 1]), 0, np.array([0, 0]))
...
while len(heap) > 0:
    ...
    if i == len(grid) - 1 and j == len(grid[i]) - 1:
        print(f"p1 answer: {block.heat_loss}")
        break
